...
public class TcpGameServer
{
        ...
        while (Running)
        {
            ...
            if (_waitingLobby.Count >= _nextGame.RequiredPlayers) // enough players to play next game
            {
                while (_nextGame.MissingPlayer)
                {
                    ...
                    await PacketProcessing.SendPacket(firstPlayerInLobby, new Packet("message", $"Joined to game {_nextGame.GameIdentifier}"));
    ...
    public async Task NewConnectionHandler()
    {
        ...
        await PacketProcessing.SendPacket(newClient, new Packet("message", $"Welcome to the Game Server. Wait to join the game\n"));
    }
